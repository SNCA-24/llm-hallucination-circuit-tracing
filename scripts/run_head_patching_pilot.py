import argparse
import os
import sys
from typing import Any, Dict


sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from src.tracing.head_patching_pilot import (
    load_forward_pass_artifact,
    run_head_patching_pilot,
    select_top_layers_from_results,
)
from src.workflow.run_artifacts import (
    build_manifest,
    create_run_directory,
    short_scope_from_ids,
    summarize_run_status,
    utc_timestamp,
    write_json,
)


STAGE_NAME = "head_patching"
DEFAULT_RUNS_ROOT = "outputs/runs"
MODEL_ALIAS = "llama-2-7b-chat"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the single-example head patching pilot.")
    parser.add_argument(
        "--input-pt",
        default="outputs/forward_pass_pilot.pt",
        help="Path to the forward-pass pilot .pt artifact.",
    )
    parser.add_argument(
        "--metadata",
        default="outputs/tracing_pilot_input.json",
        help="Path to the tracing metadata JSON file.",
    )
    parser.add_argument(
        "--layer-results",
        default="outputs/layer_patching_results.jsonl",
        help="Path to layer patching JSONL used to select top layers when --layers is omitted.",
    )
    parser.add_argument(
        "--layers",
        nargs="+",
        type=int,
        default=None,
        help="Optional explicit layer indices to patch, e.g. --layers 30 29 31.",
    )
    parser.add_argument(
        "--runs-root",
        default=DEFAULT_RUNS_ROOT,
        help="Root directory for run-scoped artifacts.",
    )
    parser.add_argument(
        "--output-jsonl",
        default=None,
        help="Optional backward-compatible JSONL output path. Defaults to the run folder.",
    )
    parser.add_argument(
        "--output-doc",
        default=None,
        help="Optional backward-compatible Markdown output path. Defaults to the run folder.",
    )
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    artifact = load_forward_pass_artifact(args.input_pt)
    example_id = str(artifact.get("example_id", "")).strip()
    if not example_id:
        raise ValueError("Forward-pass artifact is missing example_id.")

    target_layers = args.layers
    if target_layers is None:
        target_layers = select_top_layers_from_results(args.layer_results, example_id=example_id)

    run_id, run_dir = create_run_directory(
        args.runs_root,
        STAGE_NAME,
        short_scope_from_ids([example_id]),
    )
    output_jsonl_path = args.output_jsonl or str(run_dir / "head_patching_results.jsonl")
    output_doc_path = args.output_doc or str(run_dir / "head_patching_summary.md")
    artifact_paths = {
        "forward_pass_artifact": args.input_pt,
        "metadata": args.metadata,
        "layer_results": args.layer_results,
        "head_patching_results": output_jsonl_path,
        "head_patching_summary_md": output_doc_path,
    }

    started_at = utc_timestamp()
    manifest = build_manifest(
        run_id=run_id,
        stage_name=STAGE_NAME,
        started_at=started_at,
        status="running",
        input_registry_path="outputs/examples_registry.csv",
        target_example_ids=[example_id],
        code_entrypoint="scripts/run_head_patching_pilot.py",
        model_alias=MODEL_ALIAS,
        artifact_paths=artifact_paths,
        notes=[
            "Head patching only. Ablation stage was not run.",
            "One attention head at a time is patched from corrupted run into clean run at the aligned causal prediction position.",
        ],
        backward_compatibility_mode=bool(args.output_jsonl or args.output_doc),
    )
    write_json(run_dir / "manifest.json", manifest)

    processed_success: list[Dict[str, Any]] = []
    processed_failed: list[Dict[str, Any]] = []
    try:
        result = run_head_patching_pilot(
            input_pt_path=args.input_pt,
            metadata_path=args.metadata,
            layer_results_path=args.layer_results,
            output_jsonl_path=output_jsonl_path,
            output_doc_path=output_doc_path,
            model_alias=MODEL_ALIAS,
            target_layers=target_layers,
        )
        processed_success.append(
            {
                "example_id": result["summary"]["example_id"],
                "results_path": result["results_path"],
                "doc_path": result["doc_path"],
                "target_layers": result["summary"]["target_layers"],
            }
        )
    except Exception as exc:
        processed_failed.append({"example_id": example_id, "reason": str(exc)})

    status = summarize_run_status(processed_success, processed_failed)
    finished_at = utc_timestamp()
    summary = {
        "run_id": run_id,
        "stage_name": STAGE_NAME,
        "status": status,
        "finished_at": finished_at,
        "model_alias": MODEL_ALIAS,
        "artifact_paths": artifact_paths,
        "target_example_ids": [example_id],
        "retained_example_ids": [item["example_id"] for item in processed_success],
        "target_layers": target_layers,
        "processed_success": processed_success,
        "processed_failed": processed_failed,
        "skipped_already_done": [],
        "skipped_ineligible": [],
        "ablation_run": False,
    }
    write_json(run_dir / "summary.json", summary)
    manifest["finished_at"] = finished_at
    manifest["status"] = status
    write_json(run_dir / "manifest.json", manifest)

    print(f"Run ID: {run_id}")
    print(f"Run status: {status}")
    print(f"Run directory: {run_dir}")
    print(f"Target layers: {', '.join(str(layer) for layer in target_layers)}")
    print(f"Results JSONL: {output_jsonl_path}")
    print(f"Markdown summary: {output_doc_path}")
    print(f"Example retained: {example_id if processed_success else 'none'}")
    if processed_failed:
        raise SystemExit(1)


if __name__ == "__main__":
    main()
