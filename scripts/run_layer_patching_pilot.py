...
from src.tracing.layer_patching_pilot import load_forward_pass_artifact, run_layer_patching_pilot
...
STAGE_NAME = "layer_patching"
DEFAULT_RUNS_ROOT = "outputs/runs"
MODEL_ALIAS = "llama-2-7b-chat"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the single-example layer patching pilot.")
    parser.add_argument(
        "--input-pt",
    ...
    parser.add_argument(
        ...
        help="Path to tracing metadata JSON with divergence fields.",
    )
    parser.add_argument(
        "--runs-root",
...
def main() -> None:
    ...
    output_jsonl_path = args.output_jsonl or str(run_dir / "layer_patching_results.jsonl")
    output_doc_path = args.output_doc or str(run_dir / "layer_patching_summary.md")
    artifact_paths = {
        "forward_pass_artifact": args.input_pt,
        "metadata": args.metadata,
        "layer_patching_results": output_jsonl_path,
        "layer_patching_summary_md": output_doc_path,
    }
    ...
    manifest = build_manifest(
        ...
        code_entrypoint="scripts/run_layer_patching_pilot.py",
        model_alias=MODEL_ALIAS,
        artifact_paths=artifact_paths,
        notes=[
            "Layer patching only. Head patching and ablation stages were not run.",
            "Corrupted-run hidden states are patched into the clean run at the aligned causal prediction position.",
        ],
    ...
    try:
        result = run_layer_patching_pilot(
